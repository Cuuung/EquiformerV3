"""Dynamic=True gate (base / non-DeNS): single-kernel assertion for the
conservative-force compile path with compile_dynamic=True.

Verifies the DISTINGUISHING property of dynamic=True over bucketing: 3 distinct
system sizes (different natoms AND nedges) share ONE compiled kernel — the
CompiledForceRegion cache stays at size 1 throughout (cache_size growth => FAIL).

Also checks:
  (2) force / stress are finite and numerically consistent with eager on each shape
      (tolerance <1e-5; probe saw ~1e-10 for base — well within budget).
  (3) shapes 2+ run in <2s (proxy for zero recompile; first compile ~120s is OK).
...
def build_model(seed: int = 42):
    ...
    return EquiformerV3_OC(**_CFG).to(DEVICE).to(DTYPE).train()
...
def main():
    ...
    print("=" * 70)
    print(f"Dynamic=True gate [base]  device={DEVICE}  torch={torch.__version__}")
    print("=" * 70)
