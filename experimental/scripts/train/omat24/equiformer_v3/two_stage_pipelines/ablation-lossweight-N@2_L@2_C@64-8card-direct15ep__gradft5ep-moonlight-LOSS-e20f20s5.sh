#!/usr/bin/env bash
###############################################################################
# LOSS-WEIGHT ABLATION -- run B (ABLATED E:F:S = 20:20:5, paper/literature config)
#
...
#   ablation-lossweight-...-LOSS-e5f10s100.sh   (E:F:S = 5:10:100, current baseline)
...
python -c "import inspect, fairchem.core.trainers.base_trainer as bt; assert 'HybridMuon' in inspect.getsource(bt), 'WRONG fairchem imported (no HybridMuon branch): '+bt.__file__; print('[abl-B] fairchem OK:', bt.__file__)"

############################## EDIT THESE #####################################
RUN_DIR='/mnt/afs/share/checkpoint/equiformerV3/yaolekai'

# --- Stage 1: MOONLIGHT direct pretrain (--amp ON), loss 20:20:5 -------------
DIRECT_CFG='experimental/configs/omat24/mptrj/experiments/direct/equiformer_v3_N@2_L@2_C@64_epochs@15-bs@64x8_hybridmuon-moonlight-mlr@4e-4-alr@2e-4-wd@1e-3-warmup@0.5_dens-no-stress_loss-e20-f20-s5_ablation.yml'
DIRECT_ID='abl_N2L2C64_direct_15ep_moonlight_mlr4e-4_loss-e20f20s5'

# --- Stage 2: MOONLIGHT grad-ft finetune (fp32, NO --amp), loss 20:20:5 ------
GRADFT_CFG='experimental/configs/omat24/mptrj/experiments/gradient/equiformer_v3_grad-finetune_N@2_L@2_C@64_pt-dens-ft-no-reg_hybridmuon-moonlight-mlr@1.5e-4-alr@5e-5-epochs@5-bs@16x8x4-maxatoms150-wd@1e-3_loss-e20-f20-s5_ablation.yml'
GRADFT_ID='abl_N2L2C64_gradft_5ep_moonlight_mlr1.5e-4_maxatoms150_bs16x8x4_loss-e20f20s5'
##############################################################################
...
###############################################################################
echo "[abl-B] ===== Stage 1: MOONLIGHT DIRECT pretrain 15ep ($DIRECT_ID) ====="
torchrun "${TORCHRUN_COMMON[@]}" \
  my_main.py --mode train --config-yml "$DIRECT_CFG" --amp \
  "${MAIN_COMMON[@]}" \
  --identifier "$DIRECT_ID"

echo "[abl-B] Stage 1 finished. Locating its best_checkpoint.pt ..."
CKPT=$(ls -dt "$RUN_DIR"/checkpoints/*-"$DIRECT_ID"/best_checkpoint.pt 2>/dev/null | head -n1 || true)
if [[ -z "${CKPT:-}" || ! -s "$CKPT" ]]; then
  echo "[abl-B] ERROR: could not find a non-empty best_checkpoint.pt for identifier '$DIRECT_ID'." >&2
  exit 1
fi
echo "[abl-B] Using pretrained weights: $CKPT"

###############################################################################
# Stage 2: GRAD-FINETUNE  (fp32, NO --amp; overrides yml's load_pretrained_weights)
###############################################################################
echo "[abl-B] ===== Stage 2: MOONLIGHT GRAD-FT finetune 5ep ($GRADFT_ID) ====="
torchrun "${TORCHRUN_COMMON[@]}" \
...
echo "[abl-B] ===== DONE (loss 20:20:5). Stage-1 ckpt: $CKPT ====="
