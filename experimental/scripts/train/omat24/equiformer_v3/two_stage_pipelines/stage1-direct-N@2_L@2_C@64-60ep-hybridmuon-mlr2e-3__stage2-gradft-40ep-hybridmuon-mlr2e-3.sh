...
#            -> 16 GPU = global 512 (holds the 60ep recipe's lr pairing). muon_lr=2e-3.
#   Stage-2: HybridMuon gradient-force finetune, 40ep, per-GPU 16
#            -> 16 GPU = global 256. fp32, no --amp. muon_lr=2e-3.
#
# A/B Muon counterpart of the AdamW 60+40 (direct 60ep + gradft lr@5e-5-epochs@40) run.
# Everything is identical to that AdamW pipeline EXCEPT the optimizer block (HybridMuon).
# muon_lr=2e-3 is the validated-safe value: the direct model block + global batch (512) are
# IDENTICAL to the 15ep run where 2e-3 ran HEALTHY (forces_mae 0.0396, no NaN); 0.01 was the
# soft-diverge value. gradft also uses 2e-3 (conservative; that stage has no Muon validation
# yet -- watch its epoch 1 for "Found nans" warnings).
#
# Chaining:
...
# muon_lr=2e-3 (validated safe; 0.01 was the soft-diverge value). global batch 32x16=512.
DIRECT_CFG='experimental/configs/omat24/mptrj/experiments/direct/equiformer_v3_N@2_L@2_C@64_rbf@10_attn-grid@14-8_ffn-grid@14_merge-ln_epochs@60-bs@32x16_hybridmuon-mlr@2e-3-alr@2e-4-wd@1e-3-warmup@0.5_dens-no-stress_loss-e5-f10-s100.yml'
DIRECT_ID='muon_N2L2C64_direct_60ep_mlr2e-3'

# --- Stage 2: HybridMuon gradient-force finetune (fp32, NO --amp) ------------
# muon_lr=2e-3, 40ep, global batch 16x16=256.
GRADFT_CFG='experimental/configs/omat24/mptrj/experiments/gradient/equiformer_v3_grad-finetune_N@2_L@2_C@64_attn-hidden@32_rbf@10_max-neighbors@300_attn-grid@14-8_ffn-grid@14_use-gate-force-head_merge-layer-norm_pt-dens-ft-no-reg_hybridmuon-mlr@2e-3-alr@5e-5-epochs@40-bs@16x16-wd@1e-3_loss-e5-f10-s100.yml'
GRADFT_ID='muon_N2L2C64_gradft_40ep_mlr2e-3'
##############################################################################
