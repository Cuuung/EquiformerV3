#!/usr/bin/env bash
###############################################################################
# DPA4 ABLATION on N2L2C64 -- arm A1-D1D4: D1 (envelope-gated softmax) + D4 (C3 cutoff envelope), packaged
#
# Packaged because both attack the SAME defect: the third derivative of the PES at the
#   cutoff. D4 removes the envelope's -210/rcut^3 third-derivative jump directly; D1 removes the
#   same third-order kink from the attention DENOMINATOR (our sum_j w_j exp_j inherits it;
#   DPA4's sum_j w_j^2 exp_j does not, since w^2 ~ (1-x)^6). kappa_SRME is an FC3 quantity, so
#   this is the lever under test. If the packaged row moves, split into D1-only / D4-only next.
#
...
python -c "import inspect, fairchem.core.trainers.base_trainer as bt; assert 'HybridMuon' in inspect.getsource(bt), 'WRONG fairchem (no HybridMuon): '+bt.__file__; print('[dpa4-A1-D1D4] fairchem OK:', bt.__file__)"

ARM='A1-D1D4'
RUN_DIR='/mnt/afs/share/checkpoint/equiformerV3/yaolekai'
DIRECT_CFG='experimental/configs/omat24/mptrj/experiments/direct/dpa4_ablation/dpa4_A1-D1D4_N@2_L@2_C@64_direct-15ep_moonshot_bf16-compile.yml'
GRADFT_CFG='experimental/configs/omat24/mptrj/experiments/gradient/dpa4_ablation/dpa4_A1-D1D4_N@2_L@2_C@64_gradft-5ep_moonshot_bf16-compile.yml'
