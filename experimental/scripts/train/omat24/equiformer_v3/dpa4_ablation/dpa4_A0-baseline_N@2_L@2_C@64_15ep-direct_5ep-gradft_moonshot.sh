#!/usr/bin/env bash
###############################################################################
# DPA4 ABLATION on N2L2C64 -- arm A0-baseline: MATCHED CONTROL -- all three DPA4 switches OFF
#
# This arm is NOT optional. The existing N2L2C64 comparator (kappa_SRME 0.4638 /
#   CPS 0.7302) was trained WITHOUT the compile+bf16+TF32 stack that every arm here uses, so it
#   is no longer a matched control. A0 re-establishes the control under the new stack. Compare
#   A0 vs 0.4638 as a side-result: it also measures what the precision stack alone costs/buys.
#
...
python -c "import inspect, fairchem.core.trainers.base_trainer as bt; assert 'HybridMuon' in inspect.getsource(bt), 'WRONG fairchem (no HybridMuon): '+bt.__file__; print('[dpa4-A0-baseline] fairchem OK:', bt.__file__)"
...
GRADFT_CFG='experimental/configs/omat24/mptrj/experiments/gradient/dpa4_ablation/dpa4_A0-baseline_N@2_L@2_C@64_gradft-5ep_moonshot_bf16-compile.yml'
